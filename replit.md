...
- **Universal Email Verification Flow**: All regular (email/password) signups require email verification before first login. Features:
  - Signup endpoints return success message only (no tokens) - prevents auto-login
  - Users redirected to `/signup-success` page with "check your email" message
  - Login flow blocks unverified users with clear error message
  - Verification email sent with 24-hour expiry token
