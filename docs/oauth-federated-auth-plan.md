...
- Mismatched redirect_uri → 400 Bad Request (do NOT redirect)
- User denies consent → redirect to `redirect_uri?error=access_denied&state={state}`
...
3. **Permissions Section**
   ...
   - List of requested scopes with friendly descriptions:
     - ✓ View your profile information (name, photo)
     - ✓ Access your email address
     - ✓ Read your role and employment status
     - ✓ Submit timesheets on your behalf
...
- If consent exists and scopes match → skip consent screen
- If consent exists but new scopes requested → show consent screen
...
4. If refresh token → invalidate entire token family
5. Return 200 OK
...
2. **OAuth Flow Diagram**
   ```
   [User] → [Your App] → [Portal /oauth/authorize]
                              ↓ (user consents)
   [User] ← [Your App] ← [Portal redirects with code]
                              ↓
   [Your App] → [Portal /oauth/token] (exchange code for tokens)
                              ↓
   [Your App] ← [Portal] (returns access_token, refresh_token, id_token)
                              ↓
   [Your App] → [Portal /oauth/userinfo] (get user profile)
   ```
...
2. Login button → redirects to portal
