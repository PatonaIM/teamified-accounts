...
#### CRITICAL: Module Dependencies for Authentication
```typescript
// ✅ CORRECT - Import AuthModule when using authentication
import { Module } from '@nestjs/common';
import { TypeOrmModule } from '@nestjs/typeorm';
import { AuthModule } from '../auth/auth.module'; // REQUIRED for JWT guards
...
@Module({
  imports: [
    ...
    AuthModule, // CRITICAL: Required when controllers use JwtAuthGuard or RolesGuard
  ],
...
// ❌ INCORRECT - Missing AuthModule import causes dependency resolution errors
@Module({
  imports: [
    TypeOrmModule.forFeature([FeatureEntity]),
    // Missing AuthModule - will cause "JwtTokenService not available" errors
  ],
  controllers: [FeatureController],
  providers: [FeatureService],
})
...
#### CRITICAL: DTO Validation Best Practices
```typescript
// ✅ CORRECT - Use appropriate validation decorators
...
// ❌ INCORRECT - Avoid overly restrictive decorators that cause validation failures
export class CreateFeatureDto {
  @IsDecimal({ decimal_digits: '0,2' }, { message: 'Too restrictive validation' })
...
```typescript
@Controller('v1/features') // CRITICAL: Use 'v1/' NOT 'api/v1/'
@UseGuards(JwtAuthGuard, RolesGuard) // Use both guards when needed
@ApiBearerAuth() // CRITICAL: Always use without parameters for consistency
...
#### CRITICAL: Swagger Authentication Pattern
```typescript
...
// ❌ INCORRECT - Don't use parameters with @ApiBearerAuth
@ApiBearerAuth('JWT-auth') // This creates inconsistency
@ApiTags('Feature Name')
@Controller('v1/features')
export class FeatureController {
  // ...
...
#### CRITICAL: Import Paths for Authentication
```typescript
// ✅ CORRECT - Import from common directory
import { JwtAuthGuard } from '../../common/guards/jwt-auth.guard';
import { RolesGuard } from '../../common/guards/roles.guard';
import { Roles } from '../../common/decorators/roles.decorator';
import { CurrentUser } from '../../common/decorators/current-user.decorator';

// ❌ INCORRECT - Don't import from auth directory
